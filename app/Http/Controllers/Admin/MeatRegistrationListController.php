...
class MeatRegistrationListController extends Controller
{
    ...
    public function meatRegistrationInvoice(request $request, $id, $status)
    {
         $invoice =  DB::table('meat_registration_tbl AS t1')
                        ...
                        ->orderBy('t1.id', 'DESC')
                        ->first();
        return view('admin.meat_registration.invoice',compact('invoice'));
    }
     ...
     public function ApproveMeatRegistration(request $request, $id)
    {
    	...
    	$sms = "Your application no: " . $unique_id . " for meat licence has been approved by the PMC office successfully. Please visit the PMC office for further processes, including document verification and certificate issuance. You can also check your license status on " . $domain . " CORE OCEAN.";
    ...
    public function RejectMeatRegistration(request $request, $id){
        ...
        $update = [
            'status' => 2,
            'reason_rejection_by_admin' => $request->get('reject_resion'),
    	...
    	$sms = "Your application no:- " . $unique_id . " for meat licence is rejected, Reason for rejection: " . $reason . " CORE OCEAN.";
    	$this->sendsmsnew($sms,$mob_number);
