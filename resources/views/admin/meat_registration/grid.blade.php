        ...
        <div class="block-header">
            <div class="row">
                <div class="col-lg-7 col-md-6 col-sm-12">
                    @if ($status == 0)
                        <h2>Pending Meat Application</h2>
                    @elseif ($status == 1)
                        <h2>Approve Meat Application</h2>
                    @elseif ($status == 2)
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">
                            @if ($status == 0)
                                Pending Meat Application
                            @elseif ($status == 1)
                                Approve Meat Application
                            @elseif ($status == 2)
                                Reject Meat Application
                            @endif
                        </li>
                    </ul>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="header">
                            <h2>
                                @if ($status == 0)
                                    <strong style="text-transform: capitalize;">Pending Meat Application</strong>
                                @elseif ($status == 1)
                                    <strong style="text-transform: capitalize;">Approve Meat Application</strong>
                                @elseif ($status == 2)
                                        ...
                                        @foreach ($meat_registration_list as $key => $value)
                                            <tr>
                                                ...
                                                @if($value->status == 2)
                                                  ...
                                                  @endif  
            									<td>
                                                    <a href='{{ url("/meat_registration_view/{$value->id}/{$value->status}") }}' class="btn btn-info btn-sm text-light">
                                                ...
                                                @if($value->status == 1)
                                                 <td>  
                                                    <a href='{{ url("/meat_registration_invoice/{$value->id}/{$value->status}") }}'  class="btn btn-primary waves-effect m-r-20">
                                                        Invoice
                                                    </a>
                                                </td>
                                                @endif
