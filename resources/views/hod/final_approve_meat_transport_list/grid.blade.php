                                    ...
                                    <tbody>
                                        @foreach ($meat_registration_list as $key => $value)
                                            <tr>
                                                ...
                                                @if($value->t_final_approve == '2')
                								<td>{{ $value->t_final_reason_for_rejection }}</td>
                								@endif
            									<td>
                                                    <a href='{{ url("/final_meat_transport_view/{$value->id}/{$value->t_final_approve}") }}' class="btn btn-info btn-sm text-light">
                                                        <i class="zmdi zmdi-eye"></i>
                                                    </a>
                                                </td>
                                                
                                                 <td> 
                                                <a href='{{ url("/final_meat_transport_invoice/{$value->id}/{$value->t_final_approve}") }}'  class="btn btn-primary waves-effect m-r-20">
                                                        Invoice
                                                    </a>
                                               </td>
                                               
                                                 <td>
                                                       <?php if($value->t_final_approve == 1) { ?>
                                                    <a href='{{ url("/generate_english_meat_transport_pdf_by_hod/{$value->id}/{$value->t_final_approve}") }}'
                                                        class="btn btn-danger waves-effect waves-float btn-sm waves-green">
                                                        <i class="zmdi zmdi-file"></i> English
                                                    </a>
                                                    &nbsp;&nbsp;
                                                    <a href='{{ url("/generate_marathi_meat_transport_pdf_by_hod/{$value->id}/{$value->t_final_approve}") }}'
                                                        class="btn btn-danger waves-effect waves-float btn-sm waves-green">
                                                ...
                                                <!--<td>-->
                                                <!--    <a href='{{ url("/generate_meat_registration_pdf/{$value->id}/{$value->t_final_approve}") }}' class="btn btn-danger btn-sm text-light">-->
