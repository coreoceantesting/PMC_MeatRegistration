        ...
        <div class="block-header">
            <div class="row">
                <div class="col-lg-7 col-md-6 col-sm-12">
                    @if ($status == 0)
                        <h2>Pending vehicle Renewal Application</h2>
                    @elseif ($status == 1)
                        <h2>Approve vehicle Renewal Application</h2>
                    @elseif ($status == 2)
                        <h2>Reject vehicle Renewal Application</h2>
                    @endif
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">
                            @if ($status == 0)
                                Pending vehicle Renewal Application
                            @elseif ($status == 1)
                                Approve vehicle Renewal Application
                            @elseif ($status == 2)
                                Reject vehicle Renewal Application
                            @endif
                        </li>
                    </ul>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="header">
                            <h2>
                                @if ($status == 0)
                                    <strong style="text-transform: capitalize;">Pending vehicle Renewal Application</strong>
                                @elseif ($status == 1)
                                    <strong style="text-transform: capitalize;">Approve vehicle Renewal Application</strong>
                                @elseif ($status == 2)
                                    <strong style="text-transform: capitalize;">Reject vehicle Renewal Application</strong>
                                @endif
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover data-table js-exportable">
                                    <thead>
                                        <tr>
                                            <th>Sr. No.</th>
                                            <th>vehicle Renewal Application no.</th>
                                    ...
                                    <tbody>
                                        @foreach ($meat_renewal_list as $key => $value)
                                            <tr>
                                                ...
                                                @if($value->tr_final_approve == '2')
                								<td>{{ $value->tr_final_reason_for_rejection }}</td>
                								@endif
                                                <td>
                                                    <a href='{{ url("/final_meat_transport_renewal_view/{$value->id}/{$value->tr_final_approve}") }}' class="btn btn-info btn-sm text-light">
                                                        <i class="zmdi zmdi-eye"></i>
                                                    </a>
                                                </td>
                                                
                                                 <td> 
                                                <a href='{{ url("/final_meat_transport_renewal_invoice/{$value->id}/{$value->tr_final_approve}") }}'  class="btn btn-primary waves-effect m-r-20" >
                                                        Invoice
                                                    </a>
                                               </td>
                                                 ...
                                                 <td>
                                                    ...
                                                    <?php if($value->tr_final_approve == 1) { ?>
                                                    <a href='{{ url("/generate_english_meat_transport_renewal_by_hod/{$value->id}/{$value->tr_final_approve}") }}'
                                                        class="btn btn-danger waves-effect waves-float btn-sm waves-green">
                                                        <i class="zmdi zmdi-file"></i> English
                                                    </a>
                                                    &nbsp;&nbsp;
                                                    <a href='{{ url("/generate_marathi_meat_transport_renewal_by_hod/{$value->id}/{$value->tr_final_approve}") }}'
                                                        class="btn btn-danger waves-effect waves-float btn-sm waves-green">
                                              ...
                                              <?php } ?>
                                                ...
                                                <!--<td>-->
                                                <!--    <a href='{{ url("/generate_meat_registration_pdf/{$value->id}/{$value->status}") }}' class="btn btn-danger btn-sm text-light">-->
